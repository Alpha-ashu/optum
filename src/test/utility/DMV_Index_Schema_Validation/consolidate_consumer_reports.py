...
========================================
Reads:  target/All_Responses/<CONSUMER>/ConsolidateReports/*.xlsx
...
for consumer in CONSUMERS:
    ...
    all_reports = list(consumer_reports_dir.glob("*.xlsx"))
    ...
    for report_file in all_reports:
        if report_file.name.startswith("~$"):
            continue
        # Determine var_type from filename
        fname_lower = report_file.name.lower()
        var_type = "Summary"
        if "hospital" in fname_lower:
            var_type = "Hospital"
        elif "physician" in fname_lower:
            var_type = "Physician"
        ...
        try:
            ...
            target_sheet = None
            if "Validation Results" in xls.sheet_names:
                target_sheet = "Validation Results"
            elif "Final Consolidated Data" in xls.sheet_names:
                target_sheet = "Final Consolidated Data"
        ...
        for consumer in CONSUMERS:
            for var_type in VAR_TYPES:
                ...
                if subset.empty:
                    continue
                total = len(subset)
                status_col = subset.get("Match Status", pd.Series())
                matched = (status_col == "Matched").sum()
                partial = (status_col == "Partially Matched").sum()
                not_matched = (status_col == "Not Matched").sum()
