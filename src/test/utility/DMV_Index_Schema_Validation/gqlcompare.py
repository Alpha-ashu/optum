"""
GQL vs Alex Custom Resolver Validation & Comparison
===================================================
Part of DMV Index Schema Validation Framework.
Supports summation (+) aggregation on paths and custom resolver comparison.
"""
...
var = sys.argv[1].strip() if len(sys.argv) > 1 and sys.argv[1].strip() else "ClaimTransactionIdentifier"
...
alex_dir = os.path.join(BASE_DIR, 'target', 'All_Responses', 'Alex_Responses')
ppkg_dir = os.path.join(BASE_DIR, 'target', 'All_Responses', 'GQL_Responses')
...
def compare_with_mapping(alex_data: Any, ppkg_data: Any, mapping_path: str, member_number: str, claimTransactionIdentifier: str) -> List[Dict[str, Any]]:
    ...
    for ppkg_path, alex_path in mapping:
        canonical_val = extract_canonical(str(ppkg_path))
        node = extract_node_from_path(str(ppkg_path))
        ppkg_path_wild = wildcard_path(str(ppkg_path))

        if '+' in str(alex_path):
            alex_pointers = [p.strip() for p in str(alex_path).split('+')]
            alex_vals_total = 0.0
            ...
            if not ppkg_vals:
                ...
                rows.append({
                    "SubscribeID": member_number, "claimTransactionIdentifier": claimTransactionIdentifier, "Canonical": canonical_val,
            ...
            alex_path_wild = wildcard_path(str(alex_path))
            ...
            for base_path in all_base_paths:
                ...
                for p_path_item, p_val in ppkg_list:
                    for a_path_item, a_val in alex_list:
                        ...
                        rows.append({
                            ...
                            "Canonical": canonical_val, "Node": node, "HCP GQL Schema Path": p_path_item,
                ...
                if not ppkg_list and alex_list:
                    for a_path_item, a_val in alex_list:
                        ...
                        rows.append({
                            "SubscribeID": member_number, "claimTransactionIdentifier": claimTransactionIdentifier,
                            "Canonical": canonical_val, "Node": node, "HCP GQL Schema Path": ppkg_path,
                            "Custom Resolver Schema Path": a_path_item,
                            "HCP GQL Value": p_out, "Custom Resolver Value": a_out, "Status": status,
                        })
    return rows
...
def main():
    logging.info("Starting GQL compare validation...")

    # First attempt: delegate to Schema_Validation if available
    try:
        import Schema_Validation as _sv
        if var in _sv.CONSUMER_CONFIG:
            logging.info(f"Delegating to Schema_Validation engine for '{var}'...")
            res = _sv.run(var)
            if res is not None:
                return
    except Exception as ex:
        logging.warning(f"Schema_Validation run could not proceed ({ex}), falling back to direct compare.")

    # Locate claims CSV
    claims_csv_candidates = [
        os.path.join(BASE_DIR, 'src', 'test', 'resources', 'testdata', 'gql_alex', 'claimTranscation_TestData.csv'),
        os.path.join(BASE_DIR, 'src', 'test', 'resources', 'testdata', 'hcp_gql', 'hcp_alex_testdata.csv'),
    ]
    claims_csv_path = next((p for p in claims_csv_candidates if os.path.exists(p)), None)

    # Locate mapping sheet
    mapping_candidates = [
        os.path.join(BASE_DIR, 'src', 'test', 'resources', 'json_mapping', 'ppkg_alex', 'ppkg_alex_gql', f"hcp_alex_{var}.xlsx"),
        os.path.join(BASE_DIR, 'src', 'test', 'resources', 'json_mapping', 'ppkg_alex', 'ppkg_mes_gql', f"hcp_mes_{var}.xlsx"),
        os.path.join(BASE_DIR, 'src', 'test', 'utility', 'validation_mapping', 'alex_ppkg_cosmos', 'gql_customResolver', 'gql_customresolver.xlsx'),
    ]
    mapping_sheet_path = next((p for p in mapping_candidates if os.path.exists(p)), None)

    if not claims_csv_path or not os.path.exists(claims_csv_path):
        logging.warning(f"Claims CSV not found: {claims_csv_candidates}")
        return
    if not mapping_sheet_path or not os.path.exists(mapping_sheet_path):
        logging.warning(f"Mapping sheet not found: {mapping_candidates}")
        return

    try:
        df_claims = pd.read_csv(claims_csv_path)
    except Exception as e:
        logging.error(f"Error reading claims file: {e}")
        return
    ...
    for _, row in df_claims.iterrows():
        ...
        cti = str(row.get('claimTransactionIdentifier', ''))
        ...
        if not os.path.exists(alex_file) or not os.path.exists(ppkg_file):
            continue
        ...
        rows = compare_with_mapping(alex_data, ppkg_data, mapping_sheet_path, member_number, cti)
        all_rows.extend(rows)

    if not all_rows:
        logging.info("No rows compared.")
        return
