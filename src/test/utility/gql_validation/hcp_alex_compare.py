...
output_file = os.path.join(output_dir, f"{canonical}_hcp_gql_validated_report.xlsx")
...
def extract_cti_from_filename(filename: str) -> Optional[str]:
    # Matches: 01_KEN_73848570_0_1_20250626_8421981_AlexResponse.json
    match = re.match(r'(?:\d+_)?([A-Z]+)_(\d+)_(\d+)_(\d+)_(\d+)_(\d+)_.*\.json', filename)
    if match:
...
def discover_files() -> Tuple[Dict[str, Dict], Dict[str, Dict]]:
    ...
    alex_map: Dict[str, Dict[str, Any]] = {}
    ...
    if os.path.exists(alex_dir):
        for filename in os.listdir(alex_dir):
            if filename.endswith('.json'):
                file_path = os.path.join(alex_dir, filename)
                try:
                    with open(file_path, 'r', encoding='utf-8') as f:
                        data = json.load(f)
                        cti = extract_cti_from_json(data) or extract_cti_from_filename(filename)
                        if cti:
                            alex_map[cti] = {'file': filename, 'data': data}
    ...
    logging.info(f"HCP CTIs: {list(hcp_map.keys())}")
    logging.info(f"ALEX CTIs: {list(alex_map.keys())}")
    return hcp_map, alex_map
...
def substitute_indices(alex_path: str, hcp_concrete_path: str, hcp_template_path: str) -> str:
    if '*' not in alex_path:
        return alex_path
    hcp_parts = hcp_concrete_path.strip('/').split('/')
    hcp_template_parts = hcp_template_path.strip('/').split('/')
    alex_parts = alex_path.strip('/').split('/')
...
def classify_status(hcp_exists: bool, hcp_val: Any, alex_exists: bool, alex_val: Any) -> str:
    if hcp_exists and hcp_val is None:
        return "Null Value"
    if alex_exists and alex_val is None:
        return "Null Value"
    if not hcp_exists and not alex_exists:
        return "No Value"
    if not hcp_exists:
        return "Missing in HCP"
    if not alex_exists:
        return "Missing in ALEX"
    if strict_equals(hcp_val, alex_val):
        return "Matched"
    return "Different"
...
def load_mapping_sheet(mapping_path: str) -> pd.DataFrame:
    ...
    for col in df.columns:
        ...
        if 'hcp' in col_lower and 'path' in col_lower:
            hcp_col = col
        elif 'alex' in col_lower and 'path' in col_lower:
            alex_col = col
        elif 'canonical' in col_lower:
            canonical_col = col
    if not hcp_col or not alex_col:
        hcp_col = df.columns[0]
        alex_col = df.columns[1]
    result = df[[hcp_col, alex_col]].copy()
...
def compare_single_cti(cti: str, hcp_data: Any, alex_data: Any, mapping_df: pd.DataFrame) -> List[Dict]:
    rows: List[Dict[str, Any]] = []
    for _, map_row in mapping_df.iterrows():
        hcp_template = str(map_row['HCP_Path']).strip()
        alex_template = str(map_row['ALEX_Path']).strip()
        ...
        for hcp_concrete in hcp_concrete_paths:
            if '*' in hcp_concrete:
                ...
                continue
            alex_concrete = substitute_indices(alex_template, hcp_concrete, hcp_template)
            if '*' in alex_concrete:
                logging.debug(f"Skipping ALEX path with unresolved wildcards: {alex_concrete}")
                continue
            node = extract_node_from_path(hcp_concrete)
            hcp_exists, hcp_val = get_value_at_path(hcp_data, hcp_concrete)
            alex_exists, alex_val = get_value_at_path(alex_data, alex_concrete)
            status = classify_status(hcp_exists, hcp_val, alex_exists, alex_val)
            rows.append({
                'claimTransactionIdentifier': cti,
                'Canonical': canonical_val,
                'Node': node,
                'HCP Path': hcp_concrete,
                'ALEX Path': alex_concrete,
                'HCP Value': hcp_val if hcp_exists else '',
                'ALEX Value': alex_val if alex_exists else '',
                'Status': status
            })
    return rows
...
def build_response_schema(df_report: pd.DataFrame) -> pd.DataFrame:
    ...
    for (hcp_path, alex_path), group in grouped:
        hcp_norm = normalize_path(hcp_path)
        alex_norm = normalize_path(alex_path)
        hcp_present_mask = ~group['Status'].isin(['Missing in HCP', 'No Value'])
        alex_present_mask = ~group['Status'].isin(['Missing in ALEX', 'No Value'])
        hcp_types = group.loc[hcp_present_mask, 'HCP Value'].apply(type_name)
        alex_types = group.loc[alex_present_mask, 'ALEX Value'].apply(type_name)
        schema_rows.append({
            ...
            'ALEX Types Observed': ', '.join(sorted(set(alex_types.tolist()))) if len(alex_types) > 0 else '',
            'HCP Present Count': int(hcp_present_mask.sum()),
            'ALEX Present Count': int(alex_present_mask.sum()),
...
def main():
    ...
    try:
        ...
        return
    hcp_map, alex_map = discover_files()
    common_ctis = set(hcp_map.keys()) & set(alex_map.keys())
    ...
    for cti in sorted(common_ctis):
        ...
        alex_data = alex_map[cti]['data']
        rows = compare_single_cti(cti, hcp_data, alex_data, mapping_df)
    ...
    for cti, group in df_report.groupby('claimTransactionIdentifier'):
        ...
        missing_alex_count = int((group['Status'] == 'Missing in ALEX').sum())
        no_value_count = int((group['Status'] == 'No Value').sum())
        null_value_count = int((group['Status'] == 'Null Value').sum())
        mapping_errors = 0
        relevant_total = matched_count + different_count + missing_hcp_count + missing_alex_count
        ...
        summary_data.append({
            ...
            'Missing in ALEX': missing_alex_count,
    ...
    for (hcp_path, alex_path), group in df_report.groupby(['HCP Path', 'ALEX Path']):
        hcp_normalized = re.sub(r'/\d+(?=/|$)', '/*', hcp_path)
        alex_normalized = re.sub(r'/\d+(?=/|$)', '/*', alex_path)
        ...
        stats_data.append({
            ...
            'ALEX Field Path': alex_normalized,
