...
#    * def request_body = row.requestBody
  ...
  Scenario Outline: Claim UPM details Hospital request validation
    ...
    * def perfData = { consumer: 'PTRCR', feature: 'upm_physician', scenario: 'Claim UPM details Hospital request validation', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    * print('********************',[testcase],[memberId],[claimIdentifier],'[COMPLETED] ******************************')
    * def filename = 'All_Responses/PTRCR/UPM_Responses/Physician/' + memberId + '_' + claimNumber + '_upm.json'
