Feature: Request Cosmos OBAPI UPM PROD Physician Details
  Background:
  ...
  Scenario Outline: Request Physician Claim UPM details request validation
    Given url api_url
    ...
    * def perfData = { consumer: 'MYUHC', feature: 'upm_physician', scenario: 'Request Physician Claim UPM details request valida', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    * print('********************',[testcase],[memberId],[claimIdentifier],'[COMPLETED] ******************************')
    * def filename = 'All_Responses/MYUHC/UPM_Responses/Physician/' + memberId + '_' + claimNumber + '_upm.json'
