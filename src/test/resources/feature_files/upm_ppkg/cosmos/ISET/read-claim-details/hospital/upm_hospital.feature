Feature: Request Cosmos ISET UPM PROD Hospital Details
  Background:
    ...
    * def request_body = row.requestBody
...
#    """
  ...
  Scenario Outline: Claim UPM details Hospital request validation
    ...
    * def perfData = { consumer: 'ISET', feature: 'upm_hospital', scenario: 'Claim UPM details Hospital request validation', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    * print('********************',[testcase],[memberId],[claimIdentifier],'[COMPLETED] ******************************')
    * def filename = 'All_Responses/ISET/UPM_Responses/Hospital/' + memberId + '_' + claimNumber + '_upm.json'
