Feature: Clink API HCP - Claims 360 Detail Search [Hospital]
  ...
  Background:
    ...
    * def api_url = 'https://api-stg.uhg.com/api/payer/claims360-detail/v1/claims/search'

  Scenario Outline: Clink API HCP - Claims 360 Detail Search Hospital validation
        ...
        """
{
    ...
    * def perfData = { consumer: 'TOPS', feature: 'ClinkAPIHCP_Hospital', scenario: 'Clink API HCP Claims 360 Detail Search Hospital', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData

    * karate.write(response, 'All_Responses/Hospital/PPKG_Responses/' + reqInvnCtlNbr + '_' + reqClmNbr + '_ppkg.json')
