Feature: Alex Claim Transaction GraphQL API Test
  ...
  Background:
    ...
    * def query = read('classpath:testdata/hcp_gql/claimTransactionProvider/claimTransactionProvider.graphql')
 ...
 """
  Scenario Outline: Send GraphQL request for claim transaction
    ...
    * def perfData = { consumer: 'ALEX', feature: 'alex_claimTransactionProvider', scenario: 'Send GraphQL request for claim transaction', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
