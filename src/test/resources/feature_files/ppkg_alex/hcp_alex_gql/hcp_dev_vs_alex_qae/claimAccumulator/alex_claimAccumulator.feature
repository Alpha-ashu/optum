Feature: Alex Claim Accumulator GraphQL API Test
  ...
  Background:
    ...
    * def query = read('classpath:testdata/hcp_gql/claimAggregates/claimsAggregates.graphql')
 ...
 """
  Scenario Outline: Send GraphQL request for Claim Accumulator
    ...
    * def perfData = { consumer: 'ALEX', feature: 'mes_claimAccumulator', scenario: 'Send GraphQL request for Claim Accumulator', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
