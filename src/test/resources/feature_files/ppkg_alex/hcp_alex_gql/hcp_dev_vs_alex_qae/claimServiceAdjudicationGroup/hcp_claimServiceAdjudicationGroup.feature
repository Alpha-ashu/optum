Feature: HCP Service Ambulance GraphQL API Test
  ...
  Scenario Outline: Send GraphQL request for Service Ambulance
    ...
    * def perfData = { consumer: 'ALEX', feature: 'hcp_claimServiceAdjudicationGroup', scenario: 'Send GraphQL request for Service Ambulance', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
