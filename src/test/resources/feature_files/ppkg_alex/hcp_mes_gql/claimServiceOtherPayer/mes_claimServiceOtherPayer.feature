Feature: ALEX Claim Service Other Payer GraphQL API Test
  ...
  Background:
    ...
    * def query = read('classpath:testdata/hcp_gql/claimServiceOtherPayer/claimServiceOtherPayer.graphql')
    ...
    * def variables =
    ...
    """
  Scenario Outline: Send GraphQL request for Claim Service Other Payer
    ...
    * def perfData = { consumer: 'ALEX', feature: 'mes_claimServiceOtherPayer', scenario: 'Send GraphQL request for Claim Service Other Payer', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
