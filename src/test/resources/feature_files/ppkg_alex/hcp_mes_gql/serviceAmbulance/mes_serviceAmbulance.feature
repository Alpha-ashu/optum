Feature: ALEX Service Ambulance GraphQL API Test
  ...
  Background:
    ...
    * def query = read('classpath:testdata/hcp_gql/serviceAmbulance/serviceAmbulance.graphql')
  ...
  Scenario Outline: Send GraphQL request for Service Ambulance
    ...
    * def perfData = { consumer: 'ALEX', feature: 'mes_serviceAmbulance', scenario: 'Send GraphQL request for Service Ambulance', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
