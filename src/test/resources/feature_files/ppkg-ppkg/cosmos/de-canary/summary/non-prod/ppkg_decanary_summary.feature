...
#    * def Claimidentifiers = row.claimNumber
  ...
  Scenario Outline: Getting Single Record Based on Claim-identifiers
    ...
    * def perfData = { consumer: 'TOPS', feature: 'ppkg_decanary_summary', scenario: 'Getting Single Record Based on Claim-identifiers', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    * print('********************', [testcase],[ICN], '[COMPLETED] **************************')
    * def filename = 'All_Responses/TOPS/Decanary_Responses/Summary/' + testcase + '_' + ICN + '_decanaryResponse.json'
