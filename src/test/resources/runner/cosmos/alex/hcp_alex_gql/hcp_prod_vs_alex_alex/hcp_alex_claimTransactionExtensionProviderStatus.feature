Feature:  HCP GQL - Alex claimTransactionExtensionProviderStatus
  Scenario: 1. Comparing the Cosmos HCP and Alex Claim Details
    ...
    * def callonceHCP = callonce read('classpath:feature_files/ppkg_alex/hcp_alex_gql/claimTransaction-extension-provider-status/hcp_claimTransaction-extension-provider-status.feature')
    * print('******************** [HCP Physician][COMPLETED] ****************************************')
    * print('******************** [Alex Physician][STARTED] *******************************************')
    * def callonceAlex = callonce read('classpath:feature_files/ppkg_alex/hcp_alex_gql/claimTransaction-extension-provider-status/alex_claimTransaction-extension-provider-status.feature')
