Feature: UPM - PPKG ISET COSMOS for PROD ENVIRONMENT V2
  ...
  Background:
    ...
    * def cleanResponseFolder =
    ...
    #
    # NOTE: This drives the DMV Index/Schema validation engine
    # (DMV_Index_Schema_Validation/Schema_Validation.py), NOT the legacy
    # upm_ppkg_validation/Main.py + report.py pair. The ISET Summary/Hospital/
    # Physician scenarios are onboarded into DMV via config
    # (validation_config/ISET/iset_ppkg_upm_*.json) whose 'name' equals the
    # varName passed here (ISET_Summary / ISET_Hospital / ISET_Physician), so a
    # single DMV call produces the full DMV report.
    * def runValidation =
    """
    function(varName) {
      var consumer = varName.split('_')[0];
      var failures = [];
      karate.log('[VALIDATION] === Generating DMV reports for:', varName, '(Consumer:', consumer, ') ===');
      try {
        karate.exec('python src/test/utility/DMV_Index_Schema_Validation/Schema_Validation.py ' + varName);
        karate.log('[VALIDATION] DMV Schema_Validation.py completed for', varName);
      } catch(e) { failures.push('Schema_Validation.py: ' + e.message); karate.log('[VALIDATION][ERROR] DMV Schema_Validation.py failed:', e.message); }
      try {
