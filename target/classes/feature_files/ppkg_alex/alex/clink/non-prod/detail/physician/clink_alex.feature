Feature: Clink API Alex - Claims 360 Detail Search [Physician]
  ...
  Scenario Outline: Clink API Alex - Claims 360 Detail Search Physician validation
        ...
        """
{
    ...
    * def perfData = { consumer: 'TOPS', feature: 'ClinkAPIAlex_Physician', scenario: 'Clink API Alex Claims 360 Detail Search Physician', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData

    * karate.write(response, 'All_Responses/Physician/Alex_Responses/' + testcase + '_' + reqClmNbr + '_alex.json')
    * print('********************', testcase, reqTin, '[COMPLETED] ******************************')

    Examples:
      | read('classpath:testdata/clink/clink_payer_physician_testdata.csv') |
