Feature: ALEX Claim Service Accumulator GraphQL API Test
  ...
  Background:
    ...
    * def query = read('classpath:testdata/hcp_gql/claimServiceAccumulator/claimServiceAccumulator.graphql')
    ...
    * def variables =
    ...
    """
  Scenario Outline: Send GraphQL request for Claim Service Accumulator
    ...
    * def perfData = { consumer: 'ALEX', feature: 'mes_claimServiceAccumulator', scenario: 'Send GraphQL request for Claim Service Accumulator', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
