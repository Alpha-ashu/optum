Feature: ALEX Claim Transaction GraphQL API Test
  ...
  Scenario Outline: Send GraphQL request for Claim Transaction
    ...
    * def perfData = { consumer: 'ALEX', feature: 'mes_claimTransactionProvider', scenario: 'Send GraphQL request for Claim Transaction', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
