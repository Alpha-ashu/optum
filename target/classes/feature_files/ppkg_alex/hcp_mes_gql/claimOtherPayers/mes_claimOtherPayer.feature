Feature: ALEX Claim Other Payers GraphQL API Test
  ...
  Background:
    ...
    * def query = read('classpath:testdata/hcp_gql/claimOtherPayers/claimOtherPayer.graphql')
    ...
    * def variables =
    ...
    """
  Scenario Outline: Send GraphQL request for Claim Other Payers
    ...
    * def perfData = { consumer: 'ALEX', feature: 'mes_claimOtherPayer', scenario: 'Send GraphQL request for Claim Other Payers', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
