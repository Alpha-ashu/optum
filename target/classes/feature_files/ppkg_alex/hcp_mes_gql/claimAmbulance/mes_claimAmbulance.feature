Feature: ALEX Claim Ambulance GraphQL API Test
  ...
  Background:
    ...
    * def query = read('classpath:testdata/hcp_gql/claimAmbulance/claimAmbulance.graphql')
    ...
    * def variables =
    ...
    """
  Scenario Outline: Send GraphQL request for Claim Ambulance
    ...
    * def perfData = { consumer: 'ALEX', feature: 'mes_claimAmbulance', scenario: 'Send GraphQL request for Claim Ambulance', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
