Feature: HCP Claim Ambulance GraphQL API Test
  ...
  Scenario Outline: Send GraphQL request for Claim Ambulance
    ...
    * def perfData = { consumer: 'ALEX', feature: 'hcp_claimAmbulance', scenario: 'Send GraphQL request for Claim Ambulance', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
