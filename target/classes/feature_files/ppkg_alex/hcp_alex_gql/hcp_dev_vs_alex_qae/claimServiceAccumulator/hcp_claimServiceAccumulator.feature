Feature: HCP Claim Service Accumulator GraphQL API Test
  ...
  Scenario Outline: Send GraphQL request for Claim Service Accumulator
    ...
    * def perfData = { consumer: 'ALEX', feature: 'hcp_claimServiceAccumulator', scenario: 'Send GraphQL request for Claim Service Accumulator', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
