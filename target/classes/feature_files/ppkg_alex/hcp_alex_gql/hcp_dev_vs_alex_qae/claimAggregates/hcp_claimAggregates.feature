Feature: HCP Claim Accumulator GraphQL API Test
  ...
  Scenario Outline: Send GraphQL request for Claim Accumulator
    ...
    * def perfData = { consumer: 'ALEX', feature: 'hcp_claimAggregates', scenario: 'Send GraphQL request for Claim Accumulator', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
