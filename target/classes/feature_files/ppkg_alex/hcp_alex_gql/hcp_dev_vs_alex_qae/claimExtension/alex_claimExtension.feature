Feature: Alex Claim Transaction GraphQL API Test
 ...
 """
  Scenario Outline: Send GraphQL request for claim transaction
    ...
    * def perfData = { consumer: 'ALEX', feature: 'alex_claimExtension', scenario: 'Send GraphQL request for claim transaction', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
