Feature: Request Cosmos ACET PayerPackage Non-prod Physician Details
  ...
  Scenario Outline: Get PayerPackage Hospital Details
    ...
    * def filename = 'All_Responses/ACET/PPKG_Responses/Physician/' + memberId + '_' + claimNumber + '_ppkg.json'
    * def perfData = { consumer: 'ACET', feature: 'ppkg_nonprod_physician', scenario: 'Get PayerPackage Hospital Details', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    * karate.write(response, filename)


    Examples:
      | read('classpath:testdata/upm_ppkg_cosmos/ACET/physician_TestData.csv') |
      ...
      Scenario Outline: NEG - 401 when Authorization header is missing
        ...
        * def claimIdentifier = <claimIdentifier>
        ...
        When method get
        * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'ACET', feature: 'ppkg_nonprod_physician', scenario: 'NEG - 401 when Authorization header is missing', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
        Examples:
      ...
      Scenario Outline: NEG - 401 when Authorization token is invalid/malformed
        ...
        * def claimIdentifier = <claimIdentifier>
        ...
        When method get
        * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'ACET', feature: 'ppkg_nonprod_physician', scenario: 'NEG - 401 when Authorization token is invalid/malf', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
        Examples:
          | testcase              | claimIdentifier         |
      ...
      Scenario Outline: NEG - 403 when consumer/environment headers are unauthorized
        ...
        * def claimIdentifier = <claimIdentifier>
        ...
        When method get
        * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'ACET', feature: 'ppkg_nonprod_physician', scenario: 'NEG - 403 when consumer/environment headers are un', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
        Examples:
          | testcase                      | claimIdentifier         |
