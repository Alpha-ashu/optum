Feature: Request Cosmos ACET UPM PROD read claim Details
  ...
  Scenario Outline: Request for UPM Physician claim details validation
    Given url api_url
    ...
    * def perfData = { consumer: 'ACET', feature: 'upm_physician', scenario: 'Request for UPM Physician claim details validation', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    * print('********************',[testcase],[memberId],[claimIdentifier],'[COMPLETED] ******************************')
    * def filename = 'All_Responses/ACET/UPM_Responses/Physician/' + memberId + '_' + claimNumber + '_upm.json'
