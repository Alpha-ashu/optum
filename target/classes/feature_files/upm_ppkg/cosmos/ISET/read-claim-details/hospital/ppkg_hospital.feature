Feature: Request Cosmos PayerPackage PROD Hospital  Details
  ...
  Scenario Outline: Getting Single Record Based on Claim-identifiers
    ...
    * def filename = 'All_Responses/ISET/PPKG_Responses/Hospital/' + testcase + '_' + claimIdentifier + '_ppkg.json'
    * def perfData = { consumer: 'ISET', feature: 'ppkg_hospital', scenario: 'Getting Single Record Based on Claim-identifiers', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
