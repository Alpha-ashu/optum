Feature: Request Cosmos PayerPackage PROD Summary Details
  ...
  Scenario Outline: Getting Single Record Based on Claim-identifiers
    ...
    * def perfData = { consumer: 'ISET', feature: 'ppkg_summary', scenario: 'Getting Single Record Based on Claim-identifiers', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    * print('********************', [testcase], [memberId], [claimIdentifier], '[COMPLETED] **************************')
    * def filename = 'All_Responses/ISET/PPKG_Responses/Summary/' + testcase + '_' + claimIdentifier + '_ppkg.json'
