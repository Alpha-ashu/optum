Feature: Request Cosmos ISET PayerPackage Non-PROD Hospital Details
  ...
  # And header x-security-context = '{"roleid": "","authorization": ""}'
    ...
    * def perfData = { consumer: 'ISET', feature: 'ppkg_nonprod_hospital', scenario: 'Get PayerPackage Hospital Details', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    * print('********************', [testcase], [memberId], [claimIdentifier], '[COMPLETED] **************************')
    * def filename = 'All_Responses/ISET/PPKG_Responses/Hospital/' + memberId + '_' + claimNumber + '_ppkg.json'
    * karate.write(response, filename)


    Examples:
      | read('classpath:testdata/upm_ppkg_cosmos/ISET/hospital_TestData.csv') |
  ...
  Scenario Outline: NEG - 401 when Authorization header is missing
    ...
    * def claimIdentifier = <claimIdentifier>
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'ISET', feature: 'ppkg_nonprod_hospital', scenario: 'NEG - 401 when Authorization header is missing', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    Examples:
  ...
  Scenario Outline: NEG - 401 when Authorization token is invalid/malformed
    ...
    * def claimIdentifier = <claimIdentifier>
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'ISET', feature: 'ppkg_nonprod_hospital', scenario: 'NEG - 401 when Authorization token is invalid/malf', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    Examples:
      | testcase              | claimIdentifier         |
  ...
  Scenario Outline: NEG - 403 when consumer/environment headers are unauthorized
    ...
    * def claimIdentifier = <claimIdentifier>
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'ISET', feature: 'ppkg_nonprod_hospital', scenario: 'NEG - 403 when consumer/environment headers are un', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    Examples:
      | testcase                      | claimIdentifier         |
  ...
  Scenario: NEG - 415 when Content-Type is not appropriate for GET
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'ISET', feature: 'ppkg_nonprod_hospital', scenario: 'NEG - 415 when Content-Type is not appropriate for', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
  ...
  Scenario: NEG - 406 when Accept is unsupported
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'ISET', feature: 'ppkg_nonprod_hospital', scenario: 'NEG - 406 when Accept is unsupported', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
  ...
  Scenario: NEG - 400/422 when required header 'search-type' is missing
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'ISET', feature: 'ppkg_nonprod_hospital', scenario: 'NEG - 400/422 when required header search-type i', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
  ...
  Scenario: NEG - 400/422 when required header 'claim-identifier' is missing
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'ISET', feature: 'ppkg_nonprod_hospital', scenario: 'NEG - 400/422 when required header claim-identifi', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
  ...
  Scenario: NEG - 400/422 when header values are invalid
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'ISET', feature: 'ppkg_nonprod_hospital', scenario: 'NEG - 400/422 when header values are invalid', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
  ...
  Scenario Outline: NEG - 400/422 when next-key is malformed JSON
    ...
    * def nextKey = <nextKey>
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'ISET', feature: 'ppkg_nonprod_hospital', scenario: 'NEG - 400/422 when next-key is malformed JSON', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    Examples:
      | testcase                    | nextKey                             |
  ...
  Scenario Outline: NEG - 400/422 when fields-response contains unknown fields
    ...
    * def fields = <fields>
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'ISET', feature: 'ppkg_nonprod_hospital', scenario: 'NEG - 400/422 when fields-response contains unknow', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    Examples:
      | testcase                         | fields          |
  ...
  Scenario Outline: NEG - 404/200 when claim-identifier does not exist
    ...
    * def claimIdentifier = <claimIdentifier>
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'ISET', feature: 'ppkg_nonprod_hospital', scenario: 'NEG - 404/200 when claim-identifier does not exist', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    Examples:
      | testcase                            | claimIdentifier         |
  ...
  Scenario: NEG - 405 when using POST (GET-only endpoint)
    ...
    When method post
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'ISET', feature: 'ppkg_nonprod_hospital', scenario: 'NEG - 405 when using POST (GET-only endpoint)', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
  ...
  Scenario: NEG - 429 when rate limit is exceeded (retry probe)
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'ISET', feature: 'ppkg_nonprod_hospital', scenario: 'NEG - 429 when rate limit is exceeded (retry probe', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
