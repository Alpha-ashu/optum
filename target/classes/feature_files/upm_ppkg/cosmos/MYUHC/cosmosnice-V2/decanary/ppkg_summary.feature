Feature: Request Cosmos PayerPackage DECANARY Summary Details
  ...
  Scenario Outline: NEG - 401 when Authorization header is missing
    * def testcase = <testcase>
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'MYUHC', feature: 'ppkg_summary', scenario: 'NEG - 401 when Authorization header is missing', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    Examples:
      | testcase               |
  ...
  Scenario Outline: NEG - 401 when Authorization token is invalid/malformed
    * def testcase = <testcase>
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'MYUHC', feature: 'ppkg_summary', scenario: 'NEG - 401 when Authorization token is invalid/malf', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    Examples:
      | testcase               |
  ...
  Scenario Outline: NEG - 403 when consumer/environment headers are unauthorized
    * def testcase = <testcase>
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'MYUHC', feature: 'ppkg_summary', scenario: 'NEG - 403 when consumer/environment headers are un', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    Examples:
      | testcase                    |
  ...
  Scenario: NEG - 415 when Content-Type is not appropriate for GET
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'MYUHC', feature: 'ppkg_summary', scenario: 'NEG - 415 when Content-Type is not appropriate for', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
  @negative @406
  Scenario: NEG - 406 when Accept is unsupported
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'MYUHC', feature: 'ppkg_summary', scenario: 'NEG - 406 when Accept is unsupported', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
  @negative @validation
  Scenario: NEG - 400/422 when required header 'search-type' is missing
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'MYUHC', feature: 'ppkg_summary', scenario: 'NEG - 400/422 when required header search-type i', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
  @negative @validation
  Scenario: NEG - 400/422 when required header 'claim-identifier' is missing
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'MYUHC', feature: 'ppkg_summary', scenario: 'NEG - 400/422 when required header claim-identifi', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
  @negative @validation
  Scenario Outline: NEG - 400/422 when header values are invalid
    * def testcase = <testcase>
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'MYUHC', feature: 'ppkg_summary', scenario: 'NEG - 400/422 when header values are invalid', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    Examples:
      | testcase                      |
  ...
  Scenario Outline: NEG - 400/422 when next-key is malformed JSON
    ...
    * def nextKeyHeader = <nextKeyHeader>
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'MYUHC', feature: 'ppkg_summary', scenario: 'NEG - 400/422 when next-key is malformed JSON', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    Examples:
  ...
  Scenario Outline: NEG - 400/422 when fields-response contains unknown fields
    ...
    * def fields = <fields>
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'MYUHC', feature: 'ppkg_summary', scenario: 'NEG - 400/422 when fields-response contains unknow', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    Examples:
      | testcase                        | fields                                     |
  ...
  Scenario Outline: NEG - 404/200 when claim-identifier does not exist
    ...
    * def claimIdentifierNeg = <claimIdentifierNeg>
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'MYUHC', feature: 'ppkg_summary', scenario: 'NEG - 404/200 when claim-identifier does not exist', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    Examples:
  ...
  Scenario: NEG - 405 when using POST (GET-only endpoint)
    ...
    When method post
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'MYUHC', feature: 'ppkg_summary', scenario: 'NEG - 405 when using POST (GET-only endpoint)', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
  ...
  Scenario: NEG - 429 when rate limit is exceeded
    ...
    When method get
    * assert responseStatus >= 200 && responseStatus < 600
    * def perfData = { consumer: 'MYUHC', feature: 'ppkg_summary', scenario: 'NEG - 429 when rate limit is exceeded', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
