Feature: Request Cosmos ISET UPM PROD Details
  Background:
  ...
  Scenario Outline: Claim UPM details Hospital request validation
    ...
    * def perfData = { consumer: 'VETSS', feature: 'upm_physician', scenario: 'Claim UPM details Hospital request validation', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    * print('********************',[testcase],[memberId],[claimIdentifier],'[COMPLETED] ******************************')
    * def filename = 'All_Responses/VETSS/UPM_Responses/Physician/' + memberId + '_' + claimNumber + '_upm.json'
