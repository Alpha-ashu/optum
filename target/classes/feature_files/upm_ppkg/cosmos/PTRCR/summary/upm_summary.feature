Feature: Request Cosmos UPM PROD Summary
  ...
  Background:
    ...
    * eval new java.io.File('target/All_Responses/PTRCR/UPM_Responses/Summary').mkdirs()
...
#    * def json_string = row.requestBody
    * def requestBody =
    """
    {
    "searchInput": {
  ...
  Scenario Outline: Claim UPM summary request validation
    ...
    * def filename = 'All_Responses/PTRCR/UPM_Responses/Summary/' + memberId + '_' + claimNumber + '_upm.json'
    * def perfData = { consumer: 'PTRCR', feature: 'upm_summary', scenario: 'Claim UPM summary request validation', endpoint: '#(karate.info.scenarioName)', method: 'POST', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
