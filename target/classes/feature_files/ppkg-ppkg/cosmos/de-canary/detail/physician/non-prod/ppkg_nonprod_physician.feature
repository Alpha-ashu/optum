Feature: Request Cosmos PayerPackage Non-PROD Physician Details
  ...
  Scenario Outline: Get PayerPackage Physician Details
    ...
    * def perfData = { consumer: 'TOPS', feature: 'ppkg_nonprod_Physician', scenario: 'Get PayerPackage Physician Details', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    * print('********************', [testcase], [ICN], '[COMPLETED] **************************')
    * def filename = 'All_Responses/TOPS/PPKG_Responses/Physician/' + testcase + '_' + ICN + '_hcp.json'
