Feature: Request Cosmos PayerPackage Non-PROD Hospital Details
  ...
  Scenario Outline: Get PayerPackage Hospital Details
    ...
    * def perfData = { consumer: 'TOPS', feature: 'ppkg_nonprod_hospital', scenario: 'Get PayerPackage Hospital Details', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
    * call read('classpath:performance/log_performance.feature') perfData
    * print('********************', [testcase], [ICN], '[COMPLETED] **************************')
    * def filename = 'All_Responses/TOPS/Decanary_Responses/Hospital/'+ testcase +'_'+ ICN + '_ppkg.json'
    * karate.write(response, filename)

    Examples:
      | read('classpath:testdata/decanary/decanarydetailhospital.csv') |
