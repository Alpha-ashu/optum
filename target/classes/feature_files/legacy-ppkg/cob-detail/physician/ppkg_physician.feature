Feature: Request Cosmos TOPS PPKG PROD read claim Details
  ...
  Background:
    ...
    * def internalReferenceIdentifier = row.internalReferenceIdentifier
  ...
  Scenario Outline: Request for PPKG Physician claim details validation
    ...
    * def filename = 'All_Responses/TOPS/PPKG_Responses/Physician/' + testcase + '_' + internalReferenceIdentifier + '_ppkg.json'
    * def perfData = { consumer: 'TOPS', feature: 'ppkg_physician', scenario: 'Request for PPKG Physician claim details validatio', endpoint: '#(karate.info.scenarioName)', method: 'GET', status: '#(responseStatus)', responseTimeMs: '#(responseTime)' }
